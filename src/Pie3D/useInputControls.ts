...
function writeChangesToUrl(key: any) {
  return (value: any) => {
...
function parseValue(value: string | boolean | null, type: string | null) {
...
function createUrlSync() {
  ...
  return (key: string, defaultValue: any, type: string | null) => ({
...
function distributePrefix(prefix: any, controlValues: any, set: any) {
...
function setPrefix(prefix: any, value: any, controlValues: any, set: any) {
  const prefixKeys = Object.keys(controlValues).filter(d =>
    d.startsWith(prefix),
  );
  if (value === 'max') {
...
const useInputControls = () => {
  ...
  const controlValuesRef: any = React.useRef();
  const {
    numSlices,
    title,
    titleOffset,
